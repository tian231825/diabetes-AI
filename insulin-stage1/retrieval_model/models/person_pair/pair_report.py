...
def _get_treatment_info(dataset, item):
    ...
    insulin = item['insulin1'].float()
    insulin_mask = item['insulin1_mask'].float()
    ...
    return {
        'raw_regimen_sequence': stage1.get('胰岛素类型', []),
        'raw_insulin_orders': stage1.get('胰岛素医嘱执行', []),
...
def generate_pair_report(
    ...
    for idx in picked_indices:
        ...
        teacher_h = teacher_infer.encode(
            ...
            item['insulin1'],
            item['drug1'],
            insulin_mask=item['insulin1_mask'],
            lengths=item['len1'],
        )[0]
