...
def _infer_d_person_from_state_dict(state_dict):
    weight = state_dict.get('encoder.net.0.weight')
...
class PersonPairSimilarityInference:
    ...
    def _prepare_person(self, person_value, person_mask):
        ...
        expected = int(self.cfg.d_person)
        if person_value.shape[-1] != expected or person_mask.shape[-1] != expected:
            raise ValueError(
                f'Person feature dimension mismatch for checkpoint {self.checkpoint_path}: '
                f'expected d_person={expected}, got person_value={person_value.shape[-1]}, '
                f'person_mask={person_mask.shape[-1]}. '
                'This usually means the checkpoint was trained with an older patient-feature definition '
