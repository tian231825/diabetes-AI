...
class PatientFeatureExtractor:
    ...
    def _load_extra_features() -> Dict[str, Dict[str, Any]]:
        extra_path = Path(__file__).resolve().parents[1] / "data" / "feature_extra.json"
    ...
    def _extract_vital_signs(self, patient_data: Dict[str, Any]) -> List[float]:
        ...
        bp_grade = self.blood_pressure_grade_from_first_course(patient_data)
        features.append(bp_grade if bp_grade is not None else None)
...
if __name__ == "__main__":
    ...
    with open("./data/data_simplified_basic.json", 'r', encoding='utf-8') as ffr:
        xx = json.load(ffr)
    p_data_dict = {}
    for k, v in xx.items():
