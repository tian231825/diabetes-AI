...
import json
from .patient_feature_v2 import PatientFeatureExtractor
from .excluded_case_ids import EXCLUDED_CASE_IDS
import logging
import random
...
class DiabetesDataset(Dataset):
    def __init__(self, cfg, mode="train"):
        ...
        if cohort_ids_path:
            ...
            if missing:
                raise ValueError(
                    f"Common cohort contains {len(missing)} IDs unavailable after S1 filtering; "
        ...
        if fixed_split_path:
            ...
            if missing:
                raise ValueError(
                    f"Fixed split {mode} contains {len(missing)} IDs unavailable in S1 dataset; "
                    f"examples: {missing[:8]}"
                )
            ...
            n_train = int(self.n_total * self.train_ratio)
            n_val = int(self.n_total * self.val_ratio)
            n_test = self.n_total - n_train - n_val
            ...
            self.filtered_indices = list(range(self.start_idx, self.start_idx + self.n))
            self.filtered_data = [self.tensor_data[idx] for idx in self.filtered_indices]
    ...
    def filter_zero_insulin_day_cases(self, data_dict):
        logging.info(
            "Stage1 empty-treatment-day case filter is disabled; empty treatment days are handled by insulin masks."
        )
        return data_dict
    ...
    def filter_excluded_case_ids(self, data_dict):
        ...
        if removed_ids:
            logging.info(
                "Dataset excluded-case filtering removed %d samples, remaining %d samples. Removed IDs: %s",
                len(removed_ids),
                len(filtered),
                removed_ids,
            )
    ...
    def analyze_stage1_admission(self, stage1_data):
        stage1_clean = self._merge_invalid_stage1_for_admission(stage1_data)
        insulin_list = stage1_clean.get("胰岛素医嘱执行", []) if isinstance(stage1_clean.get("胰岛素医嘱执行", []), list) else []
        type_list = stage1_clean.get("胰岛素类型", []) if isinstance(stage1_clean.get("胰岛素类型", []), list) else []
        bg_list = stage1_clean.get("血糖", []) if isinstance(stage1_clean.get("血糖", []), list) else []
        drug_list = stage1_clean.get("非胰岛素降糖药医嘱执行", []) if isinstance(stage1_clean.get("非胰岛素降糖药医嘱执行", []), list) else []

        anomalies = []
        length_map = {
            "bg_days": len(bg_list),
            "insulin_days": len(insulin_list),
            "type_days": len(type_list),
            "drug_days": len(drug_list),
        }
        unique_lengths = set(length_map.values())
        if len(unique_lengths) > 1:
            anomalies.append({"type": "length_mismatch", "detail": length_map})

        day_count = min(unique_lengths) if unique_lengths else 0
        insulin_list = insulin_list[:day_count]
        type_list = type_list[:day_count]
        day_summaries = self.build_stage1_day_summaries(insulin_list, type_list)
        ...
        for summary in day_summaries:
            has_basic_sc = summary["basic_sc_total"] > eps
            has_premix_sc = summary["premix_sc_total"] > eps
            has_pump = summary["pump_total"] > eps

            reasons = []
            if has_premix_sc:
                reasons.append("premix_day")
            if has_basic_sc and has_pump:
                reasons.append("basic_sc_with_pump")

            for reason in reasons:
                issue_counter[reason] = issue_counter.get(reason, 0) + 1
            ...
            resolved_types.append("basic")

        patient_legal = len(anomalies) == 0 and all(item["legal"] for item in day_issues)
        stage1_clean["胰岛素类型"] = resolved_types
        return {
            "patient_legal": patient_legal,
            "sanitized_stage1": stage1_clean,
    ...
    def construct_data(self, cfg):
        ...
        for check_id in self.ids:
            ...
            if cfg.personality == 1:
                ...
                personality_tensor = torch.zeros(cfg.d_person1)
                personality_mask = torch.ones(cfg.d_person1)
            ...
            bg_mask_new = bg1_mask[1:]
            # print(bg_mask_new)
            if cfg.remove_null and torch.any(bg_mask_new == 0):
                data_mode = "test"
            else:
                data_mode = "train/val"
            ...
            # print(len(insulin_type), len(insulin_list), check_id)
            ...
            insulin_main_2, insulin_mask_2 = self.load_insulin_daily_simplified(
                insulin_list_2, insulin_type_2, sequence_id=f"{check_id}/stage2"
            )
            
            # all_days_vectors = []
            # prev_k3_value = None
            # # if check_id == "0001414814_1259229":
            # #     print(111)
            # # print(check_id)
            # # 1. 判断整体胰岛素类型（取第一天有效类型，保证一天一种）
            # for day_idx, day_data in enumerate(insulin_list):

            #     day_type = insulin_type[day_idx]
            #     if day_type == "premix":
            #         main_vec, current_k3_value = self.process_pre_mix_insulin_one_day(day_data=day_data,
            #                                                                           prev_k3_value=prev_k3_value,
            #                                                                           insulin_mode_merge=cfg.insulin_mode,
            #                                                                           route_divide=cfg.route_divide,
            #                                                                           stage=1)
            #         flag = 1.0
            #     elif day_type == "basic":
            #         main_vec, current_k3_value = self.process_insulin_two_vectors_one_day(day_data=day_data,
            #                                                                               prev_k3_value=prev_k3_value,
            #                                                                               insulin_mode_merge=cfg.insulin_mode,
            #                                                                               route_divide=cfg.route_divide,
            #                                                                               stage=1)
            #         flag = 0.0
            #     else:
            #         raise ValueError(f"未知的胰岛素类型: {day_type}")

            #     # 展平主向量
            #     flat_vec = []
            #     for item in main_vec:
            #         if isinstance(item, list):
            #             flat_vec.extend(item)
            #         else:
            #             flat_vec.append(item)

            #     # 在前面添加标志位
            #     flat_vec_with_flag = [flag] + flat_vec
            #     all_days_vectors.append(flat_vec_with_flag)

            #     # 更新prev_k3_value用于下一天
            #     prev_k3_value = current_k3_value

            # # 转换为Tensor
            # insulin_main = torch.tensor(all_days_vectors, dtype=torch.float32)
            drug1 = self.process_drug_vectors(
                drug_list_1,
                mode=cfg.drug_mode,
                allow_empty_placeholder=True,
            )
            drug2 = self.process_drug_vectors(self.data[check_id]["c_pep_after"]["非胰岛素降糖药医嘱执行"], mode=cfg.drug_mode)
            stage2_len = min(int(bg2.size(0)), int(insulin_main_2.size(0)), int(drug2.size(0)))
            stage2_max_days = int(getattr(cfg, "stage2_max_days", 10))
            ...
            # d_insulin = 5 if cfg.insulin_mode == 0 else 4
            # d_insulin_route_stage = int((insulin_main.shape[-1] -1)/d_insulin)
            d_insulin = insulin_main_1.shape[-1]
            # print(d_insulin)
            result_dict = {
                "check_id": check_id,
                "person_value": personality_tensor,  # 个人属性特征
                ...
                "drug2": drug2,
                "len2": stage2_len,
                # "bread_insulin_gt": insulin_main[-1],
                "d_insulin": d_insulin,
                # "d_insulin_route_stage": d_insulin_route_stage,
