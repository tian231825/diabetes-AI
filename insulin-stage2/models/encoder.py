...
class Encoder(nn.Module):
    ...
    def forward(self, batch):
        ...
        for key in ["bg1", "bg1_mask", "insulin1", "insulin1_mask", "drug1", "bg2", "bg2_mask", "insulin2", "insulin2_mask", "drug2", "person_value", "person_mask"]:
            if key in batch and torch.is_tensor(batch[key]):
                batch[key] = batch[key].to(device)

        seq_suffix = "2" if "bg2" in batch and "insulin2" in batch else "1"
        bg = batch[f"bg{seq_suffix}"]
        bg_mask = batch[f"bg{seq_suffix}_mask"]
        insulin = batch[f"insulin{seq_suffix}"]
        insulin_mask = batch[f"insulin{seq_suffix}_mask"]
        drug = batch.get(f"drug{seq_suffix}")
        ...
        real_lengths = batch.get("real_lengths", batch.get(f"len{seq_suffix}", None))

        person_emb = self.person_embedding(person_value, person_mask)
