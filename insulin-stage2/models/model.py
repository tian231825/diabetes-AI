...
from retrieval_model.models.person_pair.inference import (
    PersonPairSimilarityInference,
)
...
class Mapper(nn.Module):
    ...
    def forward(self, x_in, refer_person_emb, pred_person_emb):
        ...
        stage_fusion = x_in + refer_person_emb
        x = torch.cat([stage_fusion, pred_person_emb], dim=-1)
...
class TopKMapAggregator(nn.Module):
    ...
    def forward(self, map_k, similarity):
        ...
        similarity_feat = scaled_similarity.unsqueeze(-1)
        x = torch.cat([map_k, similarity_feat], dim=-1)
        x = self.score_proj(x)
        x = self.fuse(x)
...
class TwoStageModel(nn.Module):
    def __init__(self, cfg):
        ...
        self.reference_encoder = Encoder(cfg)
        ...
        retrieval_person_dim = int(getattr(self.simPredictor.cfg, "d_person", 0))
        current_person_dim = int(getattr(cfg, "d_person", 0))
        if retrieval_person_dim != current_person_dim:
            raise ValueError(
                "Retrieval patient feature dimension mismatch: "
                f"stage2 dataset d_person={current_person_dim}, "
                f"person-pair checkpoint d_person={retrieval_person_dim}. "
                "Please retrain/update retrieval_model checkpoints after changing patient_feature_v2."
            )
    ...
    def _build_knowledge_base(self):
        ...
        self._knowledge_emb_device = None
        logging.info(
            "Knowledge base ready for stage2: %d patients, top_k=%d",
    ...
    def _ensure_knowledge_embeddings(self, device):
        ...
        with torch.no_grad():
            ...
            self._knowledge_person_emb = self.simPredictor.model.encode_person(
                person_value, person_mask
            ).detach()
    ...
    def _retrieve_topk_indices(self, batch):
        ...
        for i in range(batch_size):
            scores = self._score_against_knowledge_base(
                batch["person_value"][i], batch["person_mask"][i]
            )
    ...
    def _encode_reference_batch(self, reference_batch, batch_size, topk):
        outputs = self.reference_encoder(reference_batch)
        insulin_values = reference_batch["insulin2"]
        ...
        return {
            ...
            "person_emb": outputs["person_emb"].reshape(
                batch_size, topk, outputs["person_emb"].shape[1]
            ),
            "real_lengths": outputs["real_lengths"].reshape(batch_size, topk),
            "bg_values": reference_batch["bg2"].reshape(
                batch_size, topk, reference_batch["bg2"].shape[1], reference_batch["bg2"].shape[2]
            ),
    ...
    def forward(self, batch, tf_ratio, mode="train"):
        ...
        topk_indices, topk_scores = self._retrieve_topk_indices(batch)
        ...
        current_person_emb = self.reference_encoder.encode_person(
            batch["person_value"],
            batch["person_mask"],
        )
        ...
        if self.use_length_predictor:
            initial_bg, _, _, _ = self.stage2._build_admission_start_initial_bg(
                batch,
                batch["bg2"],
                batch["bg2_mask"],
            )
        ...
        outputs = self.stage2(
            ...
            mode=mode,
            tf_ratio=tf_ratio,
            max_len=None if self.training else self.cfg.max_seq_len,
            enable_early_stop=not self.training,
