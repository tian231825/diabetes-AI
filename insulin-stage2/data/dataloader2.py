...
class DiabetesDataset(Dataset):
    ...
    def _convert_stage2_day_to_stage1_day(self, day):
        converted = {slot: {} for slot in list(self.KEYS_MAIN) + ["Qw", "0"] + list(self.KEYS_TEMPORAL)}
        for src_slot, dst_slot in [
            ...
            self._set_positive_value(converted[dst_slot], "皮下长效", parsed[0] + parsed[2])
            self._set_positive_value(converted[dst_slot], "皮下短效", parsed[1] + parsed[3])
    ...
    def load_insulin_daily_full_with_regimen_flags(
        ...
        for i in range(N):
            ...
            transfer_4_row = self._build_stage2_transfer_4_row(ctx, i, sequence_id=sequence_id)
            regimen_total = sum(abs(v) for v in transfer_4_row)
            ...
            if self.insulin_transfer_4:
                flag_value = 1.0 if typ == "premix" else 0.0
                row = [flag_value] + transfer_4_row
                mask = [0.0] + [1.0] * 4
            else:
                _, basic_sc_raw = self._build_stage2_basic_rows(ctx, i)
                premix_sc_raw = [
                    final_breakfast[i][0] + final_qd[i][0],
                    final_breakfast[i][1] + final_qd[i][1],
                    final_dinner[i][0] + final_qn[i][0],
                    final_dinner[i][1] + final_qn[i][1],
                ]
                if typ == "basic":
