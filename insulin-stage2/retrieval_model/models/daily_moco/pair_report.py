...
from models.daily_moco.dataset import DailySimilarityDataset
...
def _get_raw_treatment_info(dataset, check_id):
    ...
    return {
        "regimen_sequence": stage2.get("胰岛素类型", []),
        "insulin_orders": stage2.get("胰岛素医嘱执行", []),
        "drug_orders": stage2.get("非胰岛素降糖药医嘱执行", []),
    }
...
def generate_pair_report(checkpoint_path, output_dir=None, mode="val", sample_size=10, seed=None, device=None):
    ...
    cfg, _ = _load_checkpoint_cfg(checkpoint_path)
    seed = int(cfg.seed if seed is None else seed)
    checkpoint_d_person = int(getattr(cfg, "d_person", 0))

    dataset = DailySimilarityDataset(cfg, mode=mode)
    if len(dataset) == 0 and mode != "train":
        dataset = DailySimilarityDataset(cfg, mode="train")
        mode = "train"
    cfg = apply_feature_dims_from_dataset(cfg, dataset)
    if checkpoint_d_person and int(cfg.d_person) != checkpoint_d_person:
        raise ValueError(
            "Retrieval checkpoint patient feature dimension mismatch: "
            f"checkpoint d_person={checkpoint_d_person}, current dataset d_person={int(cfg.d_person)}. "
            "Please retrain daily_moco / person_pair with the current patient_feature_v2."
        )
    ...
    for i, person in enumerate(report["people"], start=1):
        ...
        lines.append(f"processed drug: {person['processed_treatment']['processed_drug']}")
        lines.append(f"raw insulin orders: {person['raw_treatment']['insulin_orders']}")
        lines.append(f"raw drug orders: {person['raw_treatment']['drug_orders']}")
        lines.append("-" * 80)
